...
namespace Servico.Implementacao.DisciplinaImp
{
    public class DisciplinaTipoServico : IDisciplinaTipoServico
    {
        ...
        public Resposta<Paginacao<DisciplinaTipo>> ListarComPaginacao(Paginacao<DisciplinaTipo> entidade)
        {
            try
            {
                var resultado = _repositorio.ListarPorPaginacao(entidade);
                if (resultado.TemErro())
                    return new Resposta<Paginacao<DisciplinaTipo>>(null, resultado.GetErros());

                return new Resposta<Paginacao<DisciplinaTipo>>(resultado.GetResultado());
            }
            catch (Exception e)
            {
                return new Resposta<Paginacao<DisciplinaTipo>>(null, $"Ocorreu um erro ao listar o tipo: {e.Message}");
            }
        }
